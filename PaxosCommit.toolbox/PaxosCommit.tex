...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{\backslash}begin\ensuremath{\{itemize\}
 } \ensuremath{\backslash}item As in the specification of module
 \.{\$}\ensuremath{TwoPhase}\.{\$}, and for the same
 reasons, we let the variable \.{\$}\ensuremath{msgs}\.{\$} be the set of all
 messages that
...
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\backslash}item We do not explicitly model the receipt of
 messages. If an
 ...
 enabled when those messages are in the set \.{\$}\ensuremath{msgs}\.{\$} of
 sent messages.
...
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\backslash}item We do not model leader selection. We define
 actions that the
 current leader may perform, but do not specify who performs them.
 \ensuremath{\backslash}end\ensuremath{\{itemize\}
}%
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 As in the specification of Two-Phase commit in module
 \.{\$}\ensuremath{TwoPhase}\.{\$}, we
 have \ensuremath{RMs} spontaneously issue Prepared messages and we ignore
 \.{\$}Prepare\.{\$}
 messages.
\end{cpar}%
\end{lcom}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 maximum of those numbers, or \.{\$}\ensuremath{\.{-}1}\.{\$} if
 \.{\$}\ensuremath{S}\.{\$} is empty.
\end{cpar}%
\end{lcom}%
\@x{\@s{8.2} \.{\LET} Max [ T \.{\in} {\SUBSET} S ] \.{\defeq}}%
\@x{\@s{36.79} {\IF} T \.{=} \{ \} \.{\THEN} \.{-} 1}%
 \@x{\@s{80.77} \.{\ELSE} \.{\LET} n\@s{21.05} \.{\defeq} {\CHOOSE} n \.{\in}
 T \.{:} {\TRUE}}%
\@x{\@s{136.58} rmax \.{\defeq} Max [ T \.{\,\backslash\,} \{ n \} ]}%
 \@x{\@s{112.08} \.{\IN}\@s{4.10} {\IF} n \.{\geq} rmax \.{\THEN} n \.{\ELSE}
 rmax}%
\@x{\@s{8.2} \.{\IN} Max [ S ]}%
\@pvspace{8.0pt}%
...
\begin{lcom}{27.51}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 All we assume about the set \.{\$}\ensuremath{Majority}\.{\$} of majorities
 is that any
...
The set of all possible messages. There are messages of type
 \.{\$}\ensuremath{\@w{Commit}}\.{\$} and \.{\$}\ensuremath{\@w{Abort}}\.{\$}
 to announce the decision, as well as
 messages for each phase of each instance of \.{\$}\ensuremath{ins}\.{\$} of
 the \ensuremath{Paxos
 } consensus algorithm. The \.{\$}\ensuremath{acc}\.{\$} field indicates the
 sender of a
...
\@y{\@s{0}%
 \.{\$}\ensuremath{rmState[rm]}\.{\$} is the state of resource manager
 \.{\$}\ensuremath{rm}\.{\$}.
}%
\@xx{}%
\@x{\@s{8.2} aState ,\,\@s{11.38}}%
\@y{\@s{0}%
 \.{\$}\ensuremath{aState[ins][ac]}\.{\$} is the state of acceptor
 \.{\$}\ensuremath{ac}\.{\$} for instance
}%
\@xx{}%
\@x{\@s{53.53}}%
\@y{\@s{0}%
...
The type-correctness invariant. Each acceptor maintains the values
 \.{\$}\ensuremath{mbal}\.{\$}, \.{\$}\ensuremath{bal}\.{\$}, and
 \.{\$}\ensuremath{val}\.{\$} for each instance of the \ensuremath{Paxos}
 consensus
 algorithm.
\end{cpar}%
\end{lcom}%
...
\@x{\@s{163.64} bal\@s{8.17} \.{:} Ballot \.{\cup} \{ \.{-} 1 \} ,\,}%
 ...
 ,\,\@w{none} \} ] ] ]}%
\@x{\@s{8.2} \.{\land} msgs \.{\in} {\SUBSET} Message}%
\@pvspace{8.0pt}%
...
\@xx{}%
 \@x{\@s{8.2} \.{\land} rmState \.{=} [ rm \.{\in} RM \.{\mapsto}\@w{working}
 ]}%
\@x{\@s{8.2} \.{\land} aState\@s{7.28} \.{=} [ ins \.{\in} RM \.{\mapsto}}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\backslash}mbox\ensuremath{\{\}}\ensuremath{\backslash}hfill\ensuremath{\{\ensuremath{\backslash}large}
 \ensuremath{\ensuremath{\backslash}textbf\{The}
 \ensuremath{Actions\}\}}\ensuremath{\backslash}hfill\ensuremath{\backslash}mbox\ensuremath{\{\}
}%
\end{cpar}%
\end{lcom}%
...
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 An action expression that describes the sending of message
 \.{\$}\ensuremath{m}\.{\$}.
\end{cpar}%
\end{lcom}%
\@x{}\midbar\@xx{}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\{\ensuremath{\backslash}large}
 \ensuremath{\ensuremath{\backslash}textbf\{RM} \ensuremath{Actions\}\}
}%
\end{cpar}%
\end{lcom}%
\@x{ RMPrepare ( rm ) \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Resource manager \.{\$}\ensuremath{rm}\.{\$} prepares by sending a phase
 \ensuremath{2a} message for
 ballot number 0 with value \.{\$}\ensuremath{\@w{prepared}}.\.{\$}
\end{cpar}%
\end{lcom}%
\@x{\@s{8.2} \.{\land} rmState [ rm ] \.{=}\@w{working}}%
 \@x{\@s{8.2} \.{\land} rmState \.{'} \.{=} [ rmState {\EXCEPT} {\bang} [ rm ]
 \.{=}\@w{prepared} ]}%
 \@x{\@s{8.2} \.{\land} Send ( [ type \.{\mapsto}\@w{phase2a} ,\, ins
 \.{\mapsto} rm ,\, bal \.{\mapsto} 0 ,\, val \.{\mapsto}\@w{prepared} ] )}%
\@x{\@s{8.2} \.{\land} {\UNCHANGED} aState}%
\@pvspace{8.0pt}%
\@x{ RMChooseToAbort ( rm ) \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Resource manager \.{\$}\ensuremath{rm}\.{\$} spontaneously decides to abort.
 It may (but
 need not) send a phase \ensuremath{2a} message for ballot number 0 with value
...
\@x{\@s{8.2} \.{\land} rmState [ rm ] \.{=}\@w{working}}%
 ...
 \.{=}\@w{aborted} ]}%
 \@x{\@s{8.2} \.{\land} Send ( [ type \.{\mapsto}\@w{phase2a} ,\, ins
 \.{\mapsto} rm ,\, bal \.{\mapsto} 0 ,\, val \.{\mapsto}\@w{aborted} ] )}%
\@x{\@s{8.2} \.{\land} {\UNCHANGED} aState}%
\@pvspace{8.0pt}%
\@x{ RMRcvCommitMsg ( rm ) \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Resource manager \.{\$}\ensuremath{rm}\.{\$} is told by the leader to commit.
 When
 this action is enabled, \.{\$}\ensuremath{rmState[rm]}\.{\$} must equal
 either \.{\$}\ensuremath{\@w{prepared}}\.{\$}
 or \.{\$}\ensuremath{\@w{committed}}\.{\$}. In the latter case, the action
 leaves the state
...
\@pvspace{8.0pt}%
\@x{ RMRcvAbortMsg ( rm ) \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Resource manager \.{\$}\ensuremath{rm}\.{\$} is told by the leader to abort.
 It
 could be in any state except \.{\$}\ensuremath{\@w{committed}}\.{\$}.
\end{cpar}%
\end{lcom}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\{\ensuremath{\backslash}large}
 \ensuremath{\ensuremath{\backslash}textbf\{Leader} \ensuremath{Actions\}\}}
 \ensuremath{\backslash}vspace\ensuremath{\{.25\ensuremath{\backslash}baselineskip\}
}%
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\end{lcom}%
\@x{ Phase1a ( bal ,\, rm ) \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
If the leader times out without learning that a decision has
 been reached on resource manager \.{\$}\ensuremath{rm}\.{\$}\mbox{'}s
 prepare/abort decision, it
 can perform this action to initiate a new ballot
 \.{\$}\ensuremath{bal}\.{\$}. (Sending
 duplicate phase \ensuremath{1a} messages is harmless.)
\end{cpar}%
\end{lcom}%
 \@x{\@s{8.2} \.{\land} Send ( [ type \.{\mapsto}\@w{phase1a} ,\, ins
 \.{\mapsto} rm ,\, bal \.{\mapsto} bal ] )}%
\@x{\@s{8.2} \.{\land} {\UNCHANGED} {\langle} rmState ,\, aState {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Phase2a ( bal ,\, rm ) \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
The action in which a leader sends a phase \ensuremath{2a} message with ballot
 \.{\$}\ensuremath{bal\.{>}0}\.{\$} in instance \.{\$}\ensuremath{rm}\.{\$},
 if it has received phase \ensuremath{1b} messages for
 ballot number \.{\$}\ensuremath{bal}\.{\$} from a majority of acceptors. If
 the leader
 received a phase \ensuremath{1b} message from some acceptor that had sent a
 phase
 \ensuremath{2b} message for this instance, then
 \.{\$}\ensuremath{maxbal\.{\geq}0}\.{\$} and the value
 \.{\$}\ensuremath{val}\.{\$}
 the leader sends is determined by the phase \ensuremath{1b} messages. (If
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \.{\$}\ensuremath{val\.{=}\@w{prepared}}\.{\$}, then
 \.{\$}\ensuremath{rm}\.{\$} must have prepared.) Otherwise,
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \.{\$}\ensuremath{maxbal\.{=}\.{-}1}\.{\$} and the leader sends the value
 \.{\$}\ensuremath{\@w{aborted}}\.{\$}.
 \ensuremath{\backslash}vspace\ensuremath{\{.5\ensuremath{\backslash}baselineskip\}
}%
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\@x{\@s{91.42} \.{\THEN}\@w{aborted}}%
 ...
 \.{\mapsto} rm ,\, bal \.{\mapsto} bal ,\, val \.{\mapsto} val ] )}%
\@x{\@s{8.2} \.{\land} {\UNCHANGED} {\langle} rmState ,\, aState {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Decide \.{\defeq}}%
\begin{lcom}{8.2}%
...
\end{cpar}%
\end{lcom}%
\@x{\@s{8.2} \.{\land} \.{\LET} Decided ( rm ,\, v ) \.{\defeq}}%
\begin{lcom}{47.91}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 True iff instance \.{\$}\ensuremath{rm}\.{\$} of the \ensuremath{Paxos}
 consensus algorithm has
...
\@x{\@s{47.91} \E\, b \.{\in} Ballot ,\, MS \.{\in} Majority \.{:}}%
 ...
 \.{\mapsto} ac ] \.{\in} msgs}%
 \@x{\@s{19.31} \.{\IN} \.{\lor} \.{\land} \A\, rm \.{\in} RM \.{:} Decided (
 rm ,\,\@w{prepared} )}%
\@x{\@s{50.82} \.{\land} Send ( [ type \.{\mapsto}\@w{Commit} ] )}%
 \@x{\@s{39.71} \.{\lor} \.{\land} \E\, rm \.{\in} RM \.{:} Decided ( rm
 ,\,\@w{aborted} )}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\{\ensuremath{\backslash}large}
 \ensuremath{\ensuremath{\backslash}textbf\{Acceptor} \ensuremath{Actions\}\}
}%
\end{cpar}%
\end{lcom}%
\@x{ Phase1b ( acc ) \.{\defeq}}%
...
\@xx{}%
\@x{\@s{8.2} \.{\lor} \E\, rm \.{\in} RM \.{:} \.{\lor} RMPrepare ( rm )}%
\@x{\@s{79.26} \.{\lor} RMChooseToAbort ( rm )}%
\@x{\@s{79.26} \.{\lor} RMRcvCommitMsg ( rm )}%
\@x{\@s{79.26} \.{\lor} RMRcvAbortMsg ( rm )}%
 \@x{\@s{8.2} \.{\lor} \E\, bal \.{\in} Ballot \.{\,\backslash\,} \{ 0 \} ,\,
 rm \.{\in} RM \.{:} Phase1a ( bal ,\, rm ) \.{\lor} Phase2a ( bal ,\, rm )}%
\@x{\@s{8.2} \.{\lor} Decide}%
...
\end{lcom}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} PCSpec \.{\implies} PCTypeOK}%
\@x{}\midbar\@xx{}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
We now assert that the two-phase commit protocol implements the
 transaction commit protocol of module \ensuremath{TCommit}. The following
 statement
 defines \.{\$}\ensuremath{TC{\bang}TCSpec}\.{\$} to be the formula
 \.{\$}\ensuremath{TCSpec}\.{\$} of module \.{\$}\ensuremath{TCommit}\.{\$}.
 (The TLA\.{\$}\ensuremath{\.{\mbox{}^+}}\.{\$}
 \ensuremath{\backslash}textsc\ensuremath{\{instance\}} statement must is
 used to rename the
 operators defined in module \.{\$}\ensuremath{TCommit}\.{\$} to avoid
 possible name conflicts
 with operators in the current module having the same name.)
\end{cpar}%
\end{lcom}%
\@x{ TC \.{\defeq} {\INSTANCE} TCommit}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} PCSpec \.{\implies} TC {\bang} TCSpec}%
\@x{}\bottombar\@xx{}%
\end{document}
